...
<div class="wrap">
	...
	<div id="content">
		...
		<div class="main-content">
		  ...
		  <source src="resources/video/CartoonKid.mp4" type="video/mp4">
		  
		  <track label="English" kind="subtitles" srclang="en" src="resources/vtt/CartoonForKids.vtt" default>
      	
		  

      <p>It appears that your browser doesn't support HTML5 video. Here's a <a href="resources/video/CartoonKid.mp4">direct link to the video instead</a>.</p>
	  </video>
							</div>
		  ...
		  <track label="English" kind="subtitles" srclang="en" src="resources/vtt/CartoonForKids-test.vtt" default>
	...
	<a align="center" class="link-login" type='button' onclick="prevous();">PREV</a>
	<a  align="center" class="link-signin" type='button' onclick="next();">NEXT</a>  
	</div>
  ...
  <h4 align="center">Speech to text converter in JS</h4>
		...
		<div id="result" hidden="true" style="size:16"></div>
		...
		<script type="text/javascript">
			...
			function startConverting () {
				if('webkitSpeechRecognition' in window){
					...
					speechRecognizer.onresult = function(event){
						...
						var str = "aircraft";	/* chỉnh lại từ cho khớp */
			...
			function startConverting1 () {
				if('webkitSpeechRecognition' in window){
					...
					speechRecognizer.onresult = function(event){
						var interimTranscripts = '';
						var str = "science";	/* chỉnh lại từ cho khớp */
						for(var i = event.resultIndex; i < event.results.length; i++){
							var transcript = event.results[i][0].transcript;
							transcript.replace("\n", "<br>");
							if(event.results[i].isFinal){
			...
			function startConverting2 () {
				if('webkitSpeechRecognition' in window){
					...
					speechRecognizer.onresult = function(event){
						var interimTranscripts = '';
						var str = "career";	/* chỉnh lại từ cho khớp */
						for(var i = event.resultIndex; i < event.results.length; i++){
							var transcript = event.results[i][0].transcript;
							transcript.replace("\n", "<br>");
							if(event.results[i].isFinal){
...
<script src="resources/captionator/captionator-min.js"></script>
  ...
  function pVideo() {
      vTag.ontimeupdate = function(e) {
          ...
          if (cTime >= 11 && cTime < 28) {
              vTag.pause();
              bt.hidden =false;
              result.hidden =false;
          }else
        	  /* đoạn video bị pause -- chỉnh lại */
        	if(cTime >= 36 && cTime < 55)
        		{
        		vTag.pause();
                bt1.hidden =false;
                result.hidden =false;
        		}else
        	if(cTime >= 61 && cTime < 67)
        	{
        		vTag.pause();
                bt2.hidden =false;
                result.hidden =false;
